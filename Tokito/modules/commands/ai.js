...
module.exports = {
    ...
    async deploy({ chat, args, fonts, event }) {
        ...
        if (!query) {
            return chat.send(fonts.sans("Provide a question, like this: ai what is Nigeria?"));
        }
        ...
        try {
            ...
            if (response.data?.response) {
                ...
                if (response.data.results) {
                    const formattedResults = formatSearchResults(response.data.results, fonts);
                ...
                chat.send(fonts.sans("The API may be experiencing errors or traffic issues. Try again later."));
            }
        } catch (error) {
            console.error("AI API Error:", error);
            chat.send(fonts.sans("An error occurred while processing your request. Please try again later."));
        }
    },
};