...
const subprefixFile = path.join(
  __dirname,
  "../../../System/handler/data/subprefixes.json"
);
...
const command = {
  ...
  async deploy({ api, chat, args, event, fonts }) {
    ...
    if (subprefix.length > 10) {
      return chat.send(
        fonts.sans("Subprefix should be 10 characters or less.")
      );
    }
    ...
    if (!admins.includes(event.senderID)) {
      return chat.send(
        fonts.sans("Only group admins can change the subprefix.")
      );
    }

    saveSubprefix(event.threadID, subprefix);
    chat.send(
      fonts.sans(`Subprefix for this group has been set to: ${subprefix}`)
    );
  },
};
